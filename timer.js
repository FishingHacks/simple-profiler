...
class Timer {
    ...
    start(name) {
        ...
        function end() {
            if (obj.end !== -1)
                throw new Error(name + ' was already ended');
            obj.end = Date.now();
        }
        return end;
    }
    print() {
        ...
        const _newValues = {};
        let i = 0;
        function addProfiling(name, start, end, calledBy) {
            if (_newValues[name] === undefined)
                _newValues[name] = [];
        ...
        for (const { name, start, end, calledBy } of this._values) {
            if (end === -1)
                console.log(chalk_1.default.bold(name) + ': not ended yet');
            else
        ...
        (0, fs_1.writeFileSync)((0, path_1.join)(process.cwd(), 
        // set this to profiling-<yourappname>-
        'profiling-<yourappname>-' + makeDate() + '.json'), JSON.stringify(_newValues));
    }
}
...
function formatTimelength(length) {
    if (length < 2000)
        ...
        return (length / 3600000).toFixed(3) + 'hr';
}
function makeDate() {
    const date = new Date();
